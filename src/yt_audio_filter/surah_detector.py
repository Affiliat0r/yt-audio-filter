...
_SURAHS: List[Tuple[str, Optional[int], List[str]]] = [
    ...
    ("Al-Imran",       3,  [r"a+l?\W*[i']?mra+n", r"aal\W*[i']?mra+n"]),
    ...
    ("Ya-Sin",         36, [r"y(?:a|ā)+\W*s[ie]+n", r"yas[ie]+n"]),
    ...
    ("Al-Jathiyah",    45, [r"al\W*ja+thiya(h)?"]),
    ...
    ("Adh-Dhariyat",   51, [r"adh?\W*dha+riya+t"]),
    ...
    ("Al-Mujadilah",   58, [r"al\W*muja+di?la(h)?"]),
    ...
    ("An-Naziat",      79, [r"an\W*na+zi[’']?a+t"]),
    ("Abasa",          80, [r"(?<![a-z])[’']?abasa(?![a-z])"]),
    ("At-Takwir",      81, [r"at\W*takwi[i]?r"]),
    ("Al-Infitar",     82, [r"al\W*infitaa?r"]),
    ("Al-Mutaffifin",  83, [r"al\W*mutaffif[ei]+n"]),
    ("Al-Inshiqaq",    84, [r"al\W*inshiqaa?q"]),
    ("Al-Buruj",       85, [r"al\W*buruu?j"]),
    ("At-Tariq",       86, [r"at\W*ta+riq"]),
    ("Al-Ala",         87, [r"al\W*a[’']?la+(?![a-z])"]),
    ("Al-Ghashiyah",   88, [r"al\W*gha+shiya(h)?"]),
    ...
    ("Al-Adiyat",      100,[r"al\W*[’']?a+diya+t"]),
    ("Al-Qariah",      101,[r"al\W*qa+ri[’']?a(h)?"]),
    ("At-Takathur",    102,[r"at\W*taka+(?:th|s)ur"]),
